...
source("setup.R")
mapfile <- read.csv("data/mouse_mapfile.csv")
...
frame (`mapfile`), and the subsequent arguments are the columns to keep.

```{r, results = 'hide', purl = FALSE}
select(SampleID, Source, Genotype)
```

To choose rows based on a specific criteria, use `filter()`:

```{r, purl = FALSE}
filter(mapfile, Genotype == WT)
```
...
In the above, we use the pipe to send the `mapfile` dataset first through
`filter()` to keep rows where `Source` is equeal to BZ, then through `select()`
to keep only the `SampleID`, `Source`, and `Genotype` columns. Since `%>%` takes
...
> ### Challenge {.challenge}
>
>  Using pipes, subset the `mapfile` to include individuals collected from
>  CJS and retain only the columns `SampleID`, and `Genotype`.
...
--->
    ...
    ##  from CJS, and retain the columns `SampleID`, and `Genotype.`
...
> ### Challenge {.challenge}
>
>  Create a new data frame from the `mapfile` data that meets the following
...
In `mapfile` , the rows of `mapfile` contain the values of variables associated
...
Let's use `spread()` to transform mapfile to find the mean weight of each 
...
This yields `mapfile_gw` where the observation for each plot is spread across
...
form of `mapfile_spread`, where the genus names are column names, but we 
...
To recreate `mapfile_gw` from `mapfile_spread` we would create a key called
...
species_counts <- mapfile_complete %>%
  group_by(species_id) %>%
  tally %>%
  filter(n >= 50)

## Only keep the most common species
mapfile_complete <- mapfile_complete %>%
...
To make sure that everyone has the same dataset, check that `mapfile_complete`
has `r nrow(mapfile_complete)` rows and `r ncol(mapfile_complete)` columns by
typing `dim(mapfile_complete)`.
...
write.csv(mapfile_complete, file = "data_output/mapfile_complete.csv",
          row.names = FALSE)
```
